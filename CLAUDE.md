...
사용자가 **"작성할 예제를 알려줘"**, **"다음 예제 리스트"** 등 다음에 만들 예제를 물어보면,
**예제 리스트와 개선 리스트를 반드시 섹션을 구분하여 함께** 보여줍니다.

```
## 예제 리스트
- 메모리 `project_pending_examples.md`의 미완료 예제 항목 (우선순위/제약 포함)

## 개선 리스트
- 메모리 `project_code_improvements.md`의 미처리 개선 항목 (🔴/🟡/🟢 우선순위별)
```

- 두 섹션은 별도 제목으로 명확히 구분 (예제와 개선 항목을 한 표에 섞지 않음)
- 각 항목은 식별자/파일 경로/제약을 함께 표기
- 개선 항목이 모두 "없음"이면 개선 리스트 섹션에 "현재 미처리 개선 항목 없음"으로 표기
...
사용자가 **"NN번 항목을 진행해주세요"** (예: "23번 항목을 진행해주세요") 라고 입력하면 아래 절차를 수행합니다:

### 절차

```
[START]
1. 메모리 `project_pending_examples.md`에서 NN번 항목 확인
   - 카테고리/파일 예정 경로/내용/제약 확인
2. 예제 구현
   - "새 예제 추가 방법" 4단계 절차 준수 (ConstValue → Examples20XX → UI 파일 → Router)
3. 빌드 검증 (`./gradlew assembleDebug`)
   - 실패 시 수정 후 재빌드, 성공할 때까지 반복
4. 커밋 → `git push origin main`
5. 사전작업 절차 수행
6. 사전작업 결과 🔴/🟡/🟢 개선 항목 존재 시 → 즉시 수정 → 빌드 → 커밋 → 푸시 → 다시 5번으로
7. 사전작업 결과 모두 "없음"이면 루프 종료
8. 메모리 정리
   - `project_pending_examples.md`에서 NN번 항목을 "완료된 항목" 섹션으로 이동 (커밋 해시·파일 경로·핵심 내용 기록)
   - `MEMORY.md` 인덱스 갱신 (남은 미완료 건수, 최근 완료 항목 메모 반영)
9. 최종 결과 보고 (프로세스 검증 결과 표 포함)
[END]
```

### 주의 사항

- 각 커밋 이후 반드시 `git push origin main` 실행 (커밋과 푸시는 항상 함께).
- 사전작업 루프 중 제약 조건(버전 업그레이드 필요 등)이 있는 항목은 건너뛰고 즉시 수정 가능한 항목만 처리.
- 메모리 정리는 반드시 사전작업 루프 종료 후 마지막에 수행.

---

## 신규 발견 항목 반복 개선 규칙

사용자가 **"신규 발견 항목을 개선해주세요"** 라고 입력하면 아래 루프를 수행합니다:

### 루프 절차

```
[LOOP START]
1. 현재 메모리(project_code_improvements.md)에 개선 항목이 있으면 전부 수정
   → 빌드 검증 → 커밋 → 푸시
2. 사전작업 절차 수행 (전체 코드 스캔 + 라우팅 확인)
3. 신규 개선 항목 발견 시 → 1번으로 돌아가 반복
4. 신규 개선 항목 없음 → 루프 종료
[LOOP END]
```
...
사전작업 결과 **🔴 높음 / 🟡 중간 / 🟢 낮음 모두 "없음"** 인 경우 루프를 종료하고 아래 메시지를 출력합니다:

```
✅ 신규 발견 항목이 없습니다. 모든 개선 항목이 완료되었습니다.
```

### 주의 사항

- 각 수정 후 반드시 빌드 검증 → 커밋 → 푸시를 완료한 뒤 다음 사전작업을 수행합니다.
- 의도적으로 제외된 항목(교육용 코드 등)은 개선 대상에서 제외합니다.
- 루프 중 제약 조건(버전 업그레이드 필요 등)이 있는 항목은 건너뛰고 즉시 수정 가능한 항목만 처리합니다.
