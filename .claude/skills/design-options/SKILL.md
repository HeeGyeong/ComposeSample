...
- 예: `/design-options UpdatePersonaFragment arguments 500KB+ 경량화 — Bundle 에 뭘 남기고 뭘 뺄지`
- 예: `/design-options ChatBot 백스택 무한 누적(add()) 개선 방향`
...
const DESIGNER_LENSES = (args && args.designerLenses) || [
  ...
  { key: 'compat-first', desc: '호환 우선. 백스택/딥링크/SavedState/기존 로컬·서버 데이터와의 공존과 단계적 마이그레이션 경로를 최우선으로 설계한다.' },
]
...
const JUDGE_LENSES = [
  ...
  { key: 'compat', desc: '호환·마이그레이션 경로·롤백 가능성. 기존 백스택/딥링크/데이터와의 공존, 단계 출시와 되돌리기 용이성을 평가한다.' },
]
