...
- **Latest updates** (full history in [CHANGELOG.md](CHANGELOG.md))
  - 2026.07: New examples added (Screenshot Detection, Advanced Repository Pattern). Removed the legacy subsystem entirely (24 files + 10 Activities, incl. its RxJava dependency). Dead code/doc cleanup (45 unreferenced `*Guide.kt` files, dead Koin registrations) and an architecture cleanup (`AndroidViewModel` → plain `ViewModel` where `Application` was unused).
