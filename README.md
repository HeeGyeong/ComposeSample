...
- **최신 업데이트**
  - 2026.05: 신규 예제 추가 (Accessible Focus Indicator — 키보드/D-pad 포커스 시각화 4가지 패턴 + IndicationNodeFactory + DrawModifierNode 커스텀 인디케이션, Document Editing TextField — TextFieldState 심화로 undoState/selection 조작/AnnotatedString 미리보기/멀티 커서 시뮬레이션, Syntax Highlighting — AnnotatedString + 정규식 토크나이저 기반 Kotlin 코드 하이라이팅 미니 데모, Particle Emitter — Canvas + withFrameNanos 기반 물리 파티클 시스템(폭죽/별가루) + Canvas vs Layout 트레이드오프)
