...
- **Latest updates** (full history in [CHANGELOG.md](CHANGELOG.md))
  - 2026.08: New examples added (LazyList `contentType` Reuse-Pool Trap, Compose Grid API). Convention/structure cleanup — 16 hardcoded blog URLs replaced with the `blogUrl(postId)` helper (CONV-08) and the Shimmer sub-category parent fixed to carry the group constant (REG-DUAL-01). Corrected the "register routing" instructions across 4 documents: routing is an `ExampleUiRegistry.kt` map lookup, not an `ExampleRouter.kt` when-expression, and an unregistered type silently falls back to a Dummy screen (DOC-ROUTE-01).
